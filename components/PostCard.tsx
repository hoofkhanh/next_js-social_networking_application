...
type Posts = Awaited<ReturnType<typeof getPosts>>; // => ví dụ { id: number; title: string; }[]
type Post = Posts[number]; // => ví dụ { id: number; title: string; }
...
const PostCard = ({
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      setOptimisticLike((prev) => prev + (hasLike ? -1 : 1));
  ...
  const handledeletePost = async () => {
    ...
    try {
      ...
      else throw new Error(result?.error || "Failed to delete post");
    } catch (error) {
